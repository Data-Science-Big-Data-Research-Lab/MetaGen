...
"""
from dummy_examples import (get_categorical_domain, get_vector_domain,
                          get_all_types_domain, categorical_example_fitness,
                          vector_example_fitness, all_types_fitness)
from ml_examples import (get_knn_classifier_domain, knn_classifier_fitness,
...
from simple_examples import (get_x_minus_15_raised_to_2_domain,
...
def problem_dispatcher(example, connector = BaseConnector()):
    if example == "dummy-1":
        ...
        pass
        # problem_definition = get_lstm_domain
...
def cvoa_dispatcher(problem, fitness, iterations=5):
    strain = CVOA(problem, fitness, "Strain A", pandemic_duration=iterations)
    solution = strain.run()
    return solution
